...
class ShadowHandRope(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        # *** NEW for Rope Task ***
        self.target_displacement_positive = self.cfg["env"].get("targetDisplacementPositive", 0.15) # Default to 0.15 meters from center
        # *** End NEW ***
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        angle_x = np.pi / 2.0
        quat_x = gymapi.Quat(np.sin(angle_x / 2.0), 0, 0, np.cos(angle_x / 2.0))
        
        # Second rotation: -90 degrees around Z-axis to make it perpendicular to fingers
        angle_z = -np.pi / 2.0
        quat_z = gymapi.Quat(0, 0, np.sin(angle_z / 2.0), np.cos(angle_z / 2.0))
        
        # Combine rotations (quaternion multiplication)
        # Note: In quaternion multiplication, the rightmost rotation is applied first
        rope_start_pose.r = quat_z * quat_x
