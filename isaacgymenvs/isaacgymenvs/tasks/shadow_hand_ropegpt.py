# Adapted from shadow_hand_spin.py for ShadowHandRopeGPTGPT task
...
def compute_reward(current_palm_displacement: Tensor, target_displacement: Tensor) -> Tuple[Tensor, Dict[str, Tensor]]:
    # Parameters
    displacement_error_temperature: float = 0.1  # Temperature parameter to scale the displacement error part of the reward

    # Compute the squared error between current displacement and the target displacement
    displacement_error = torch.square(current_palm_displacement - target_displacement)

    # Using an exponential decay on the squared error to convert it into a positive reward,
    # which is higher when the error is smaller.
    reward = torch.exp(-displacement_error / displacement_error_temperature)

    # Pack individual reward components in a dict for potential debugging or analysis
    reward_components = {
        'displacement_error': displacement_error,
        'reward_from_error': reward
    }
